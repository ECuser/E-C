...
ORDERS_FILE = pick_file("Select Orders Excel")
if not ORDERS_FILE:
    raise SystemExit("No Orders file selected.")

LAUNCH_FILE = pick_file("Select Launch Plan Excel")
if not LAUNCH_FILE:
    raise SystemExit("No Launch Plan file selected.")

OUTPUT_FILE = str(Path(ORDERS_FILE).with_name("launch_check_output.xlsx"))
...
def load_excel(path, header_row=0, use_column_letters=False):
...
# LOAD FILES
# =========================
orders = load_excel(ORDERS_FILE, header_row=HEADER_ROW_ORDERS, use_column_letters=USE_COLUMN_LETTERS)
launch = load_excel(LAUNCH_FILE, header_row=HEADER_ROW_LAUNCH, use_column_letters=USE_COLUMN_LETTERS)
...
if USE_COLUMN_LETTERS:
    # Orders file
    orders_order_date_col = "D"
    orders_delivery_date_col = "E"
    orders_material_col = "F"
    orders_qty_col = "G"
    orders_sales_unit_col = "L"

    # Pas deze aan indien nodig
    orders_chain_col = "B"
    orders_country_col = "C"

    # Launch file
    launch_material_col = "A"
    launch_chain_col = "C"
    launch_country_col = "E"
    launch_start_date_col = "G"

else:
    # Orders file
    orders_order_date_col = pick_col(orders, ["ORDER DATE", "ORDER_DATE"], fallback_idx=3)
    orders_delivery_date_col = pick_col(orders, ["DELIVERY DATE", "DELIVERY_DATE"], fallback_idx=4)
    orders_material_col = pick_col(orders, ["MATERIAL", "SKU", "PRODUCT"], fallback_idx=5)
    orders_qty_col = pick_col(orders, ["ORDER QTY", "ORDER QUANTITY", "QTY", "QUANTITY", "ORDER QUANTITY (ITEM)", "ORDER QTY (ITEM)", "QUANTITY (ITEM)"])
    orders_sales_unit_col = pick_col(orders, ["SALES UNIT", "UNIT", "UOM"], fallback_idx=11)

    # Heel belangrijk: check deze 2 in je orders file
    orders_chain_col = pick_col(orders, ["CHAIN", "CLIENT", "CUSTOMER"], fallback_idx=1)
    orders_country_col = pick_col(orders, ["COUNTRY", "MARKET"], fallback_idx=2)

    # Launch file
    launch_material_col = pick_col(launch, ["MATERIAL", "SKU", "PRODUCT"], fallback_idx=0)
    launch_chain_col = pick_col(launch, ["CHAIN", "CLIENT", "CUSTOMER"], fallback_idx=2)
    launch_country_col = pick_col(launch, ["COUNTRY", "MARKET"], fallback_idx=4)
    launch_start_date_col = pick_col(launch, ["START DATE", "LAUNCH DATE", "START"], fallback_idx=6)
...
# Same combination + same order date together
orders_by_day = (
...
result["first_order_vs_expected_w1_pct"] = np.where(
    ...
    result["first_order_qty"] / result["expected_w1"],
    np.nan
)

result["actual_7d_vs_expected_w1_pct"] = np.where(
    result["expected_w1"].fillna(0) > 0,
    result["actual_qty_first_7d"] / result["expected_w1"],
    np.nan
)

result["actual_52w_vs_expected_52w_pct"] = np.where(
    result["expected_52w_total"].fillna(0) > 0,
    result["actual_qty_first_52w"] / result["expected_52w_total"],
    np.nan
)

# Meaningful launch date = first order day that reaches threshold % of expected week 1
...
orders_with_launch_check["pct_of_expected_w1"] = np.where(
    ...
    orders_with_launch_check["total_qty"] / orders_with_launch_check["expected_w1"],
    np.nan
)
...
result = result.merge(
    meaningful_launch[[
        ...
        "chain_key",
        "country_key",
        "meaningful_launch_date",
        "meaningful_launch_day_qty",
...
final_cols = [
    ...
    "country_original",
    "planned_launch_date",
    "actual_first_order_date",
    "actual_first_delivery_date",
    "meaningful_launch_date",
    "launch_date_diff_days",
    "expected_w1",
    "first_order_qty",
    "first_order_vs_expected_w1_pct",
    "actual_qty_first_7d",
    "actual_7d_vs_expected_w1_pct",
    "expected_52w_total",
    "actual_qty_first_52w",
    "actual_52w_vs_expected_52w_pct",
    "meaningful_launch_day_qty",
    "meaningful_launch_day_pct_of_expected_w1",
    "sales_unit_first_order",
    "launch_status",
]
